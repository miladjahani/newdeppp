...
/* ------------------------------------------------------------------
   Multi-user (sub-user) support — additive, phase 2.
   Two storage backends, selected by 键值配置.dbMode ('kv' default | 'd1'):
     - kv (default): array under kv-config key "mu", via existing
       /api/config endpoint. No extra bindings needed.
     - d1 (optional): a real "users" table in D1, for admins who want
       SQL-backed management at higher user counts. Requires a D1
       binding named "DB" on the deployed worker (bind it manually in
       the Cloudflare dashboard: Worker → Settings → Bindings → D1).
       Table is created automatically on first use if missing.
   Shape of one entry (same fields regardless of backend):
     { uuid, label, active: true, expire: "2026-12-31" | null,
       quotaGB: 0 | number, usedBytes: 0, telegramChatId }
   quotaGB: 0 means unlimited. expire: null means never.
   ------------------------------------------------------------------ */
let D1存储 = null;
let D1表已初始化 = false;
let D1用户缓存 = null;
let D1用户缓存时间 = 0;
const D1缓存期限 = 15000; // ms — connections re-check auth this often at most from D1

async function 初始化D1表() {
  if (!D1存储 || D1表已初始化) return;
  try {
    await D1存储.exec(
      `CREATE TABLE IF NOT EXISTS users (
        uuid TEXT PRIMARY KEY, label TEXT, active INTEGER DEFAULT 1,
        expire TEXT, quotaGB REAL DEFAULT 0, usedBytes INTEGER DEFAULT 0,
        requestLimit INTEGER DEFAULT 0, usedRequests INTEGER DEFAULT 0,
        preferredIPs TEXT, tlsOnly INTEGER DEFAULT 0,
        blockAds INTEGER DEFAULT 0, blockAdult INTEGER DEFAULT 0,
        proxy TEXT, fallback TEXT, region TEXT,
        telegramChatId TEXT
      )`
    );
    const 结构 = await D1存储.prepare('PRAGMA table_info(users)').all();
    const 现有列 = new Set(((结构 && 结构.results) || []).map(列 => 列.name));
    const 待添加列 = [
      ['requestLimit', 'INTEGER DEFAULT 0'], ['usedRequests', 'INTEGER DEFAULT 0'],
      ['preferredIPs', 'TEXT'], ['tlsOnly', 'INTEGER DEFAULT 0'],
      ['blockAds', 'INTEGER DEFAULT 0'], ['blockAdult', 'INTEGER DEFAULT 0'],
    ...
    for (const [列名, 定义] of 待添加列) {
      if (!现有列.has(列名)) await D1存储.exec(`ALTER TABLE users ADD COLUMN ${列名} ${定义}`);
    }
    D1表已初始化 = true;
  } catch (忽略) {}
}
function 使用D1模式() {
  return !!(D1存储 && 键值配置 && 键值配置.dbMode === 'd1');
}
async function D1获取用户列表(强制刷新 = false) {
  if (!强制刷新 && D1用户缓存 && Date.now() - D1用户缓存时间 < D1缓存期限) return D1用户缓存;
  try {
    const 结果 = await D1存储.prepare('SELECT * FROM users').all();
    const 行 = (结果 && 结果.results) || [];
    D1用户缓存 = 行.map(行项 => ({
      uuid: 行项.uuid, label: 行项.label, active: !!行项.active,
      expire: 行项.expire, quotaGB: 行项.quotaGB, usedBytes: 行项.usedBytes,
      requestLimit: Number(行项.requestLimit) || 0, usedRequests: Number(行项.usedRequests) || 0,
      preferredIPs: 行项.preferredIPs || '', tlsOnly: !!行项.tlsOnly,
      blockAds: !!行项.blockAds, blockAdult: !!行项.blockAdult,
      proxy: 行项.proxy || '', fallback: 行项.fallback || '', region: 行项.region || '',
      telegramChatId: 行项.telegramChatId
    }));
    D1用户缓存时间 = Date.now();
    return D1用户缓存;
  } catch (错误) {
...
async function D1写入用户(条目) {
  await 初始化D1表();
  await D1存储.prepare(
    `INSERT INTO users (
       uuid, label, active, expire, quotaGB, usedBytes, requestLimit, usedRequests,
       preferredIPs, tlsOnly, blockAds, blockAdult, proxy, fallback, region, telegramChatId
     ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
     ON CONFLICT(uuid) DO UPDATE SET label=excluded.label, active=excluded.active,
       expire=excluded.expire, quotaGB=excluded.quotaGB, usedBytes=excluded.usedBytes,
       requestLimit=excluded.requestLimit, usedRequests=excluded.usedRequests,
       preferredIPs=excluded.preferredIPs, tlsOnly=excluded.tlsOnly,
       blockAds=excluded.blockAds, blockAdult=excluded.blockAdult,
       proxy=excluded.proxy, fallback=excluded.fallback, region=excluded.region,
       telegramChatId=excluded.telegramChatId`
  ).bind(
    条目.uuid, 条目.label || '', 条目.active === false ? 0 : 1, 条目.expire || null,
    Number(条目.quotaGB) || 0, Number(条目.usedBytes) || 0,
    Number(条目.requestLimit) || 0, Number(条目.usedRequests) || 0,
    条目.preferredIPs || '', 条目.tlsOnly ? 1 : 0,
    条目.blockAds ? 1 : 0, 条目.blockAdult ? 1 : 0,
    条目.proxy || '', 条目.fallback || '', 条目.region || '', 条目.telegramChatId || null
  ).run();
  D1用户缓存 = null; // invalidate cache
}
async function D1删除用户(uuid) {
  await 初始化D1表();
  await D1存储.prepare('DELETE FROM users WHERE uuid = ?').bind(uuid).run();
  D1用户缓存 = null;
}
/* Unified accessor — call this instead of touching KV/D1 directly. */
async function 获取多用户列表Async() {
  if (使用D1模式()) return await D1获取用户列表();
  return 获取多用户列表();
}
function 获取多用户列表() {
  const 列表 = 键值配置 && 键值配置.mu;
  return Array.isArray(列表) ? 列表 : [];
}
function 多用户是否可用(条目) {
  if (!条目 || !条目.uuid) return false;
  if (条目.active === false) return false;
  if (条目.expire) {
    const 到期 = Date.parse(条目.expire);
    if (!isNaN(到期) && Date.now() > 到期) return false;
  }
  if (条目.quotaGB && Number(条目.quotaGB) > 0) {
    const 上限字节 = Number(条目.quotaGB) * 1024 * 1024 * 1024;
    if ((Number(条目.usedBytes) || 0) >= 上限字节) return false;
  }
  if (条目.requestLimit && Number(条目.requestLimit) > 0 &&
      (Number(条目.usedRequests) || 0) >= Number(条目.requestLimit)) return false;
  return true;
}
/* Returns ordered candidate tokens to try during protocol handshake:
   owner's own UUID first (zero behavior change for existing single-user
   deployments), then every currently-valid sub-user UUID. */
async function 获取候选认证令牌列表() {
  const 候选 = [认证令牌];
  for (const 条目 of await 获取多用户列表Async()) {
    if (多用户是否可用(条目) && 条目.uuid !== 认证令牌) 候选.push(条目.uuid);
  }
  return 候选;
}
/* Best-effort usage accounting: increments usedBytes for a matched
   sub-user and schedules a debounced write. Never throws — a
   failure here must not interrupt an already-authorized connection. */
let 多用户使用量待写入 = false;
async function 记录多用户流量(匹配uuid, 字节数) {
  if (!匹配uuid || 匹配uuid === 认证令牌 || !字节数) return;
  try {
    const 列表 = await 获取多用户列表Async();
    const 条目 = 列表.find(项目 => 项目.uuid === 匹配uuid);
    if (!条目) return;
    条目.usedBytes = (Number(条目.usedBytes) || 0) + 字节数;
    if (使用D1模式()) {
      await D1写入用户(条目);
      return;
    }
    if (!多用户使用量待写入) {
      ...
      queueMicrotask(async () => {
        多用户使用量待写入 = false;
        try { await 保存键值配置(); } catch (忽略) {}
      });
    }
  } catch (忽略) {}
}
async function 记录多用户请求(匹配uuid) {
  if (!匹配uuid || 匹配uuid === 认证令牌) return;
  try {
    const 列表 = await 获取多用户列表Async();
    const 条目 = 列表.find(项目 => 项目.uuid === 匹配uuid);
    if (!条目) return;
    条目.usedRequests = (Number(条目.usedRequests) || 0) + 1;
    if (使用D1模式()) await D1写入用户(条目); else await 保存键值配置();
  } catch (忽略) {}
}
async function 获取有效多用户(uuid) {
  const 条目 = (await 获取多用户列表Async()).find(项目 => 项目.uuid === uuid);
  return 条目 && 多用户是否可用(条目) ? 条目 : null;
}

/* Unified admin API for sub-user CRUD — works against KV or D1
   transparently based on 键值配置.dbMode. Consumed by admin-users.html. */
async function 处理用户API(请求) {
  const json = (数据, 状态 = 200) => new Response(JSON.stringify(数据), { status: 状态, headers: { 'Content-Type': 'application/json' } });
  if (请求.method === 'GET') {
    const 列表 = await 获取多用户列表Async();
    return json({ dbMode: (键值配置 && 键值配置.dbMode) || 'kv', d1Available: !!D1存储, users: 列表 });
  }
  if (请求.method !== 'POST') return json({ error: 'method not allowed' }, 405);
  let 主体 = null;
  try { 主体 = await 请求.json(); } catch (e) { return json({ success: false, message: 'invalid json' }, 400); }
  const 动作 = 主体 && 主体.action;
  try {
    if (动作 === 'setMode') {
      const 模式 = 主体.mode === 'd1' ? 'd1' : 'kv';
      if (模式 === 'd1' && !D1存储) return json({ success: false, message: 'D1 binding "DB" not found on this worker. Bind it first in the Cloudflare dashboard.' }, 400);
      键值配置.dbMode = 模式;
      await 保存键值配置();
      return json({ success: true, dbMode: 模式 });
    }
    if (动作 === 'upsert') {
      const 条目 = 主体.user || {};
      if (!条目.uuid) return json({ success: false, message: 'uuid required' }, 400);
      if (使用D1模式()) {
        await D1写入用户(条目);
      } else {
        const 列表 = 获取多用户列表();
        const 索引 = 列表.findIndex(项目 => 项目.uuid === 条目.uuid);
        if (索引 >= 0) 列表[索引] = { ...列表[索引], ...条目 }; else 列表.push(条目);
        键值配置.mu = 列表;
        await 保存键值配置();
      }
    ...
    if (动作 === 'delete') {
      const uuid = 主体.uuid;
      if (!uuid) return json({ success: false, message: 'uuid required' }, 400);
      if (使用D1模式()) {
        await D1删除用户(uuid);
      } else {
        const 列表 = 获取多用户列表().filter(项目 => 项目.uuid !== uuid);
        键值配置.mu = 列表;
        await 保存键值配置();
      }
      return json({ success: true });
    ...
    return json({ success: false, message: String(错误 && 错误.message || 错误) }, 500);
  }
}

/* ------------------------------------------------------------------
   Telegram bot — phase: bot integration.
   Config lives entirely in KV under key "tg": { token, secret }.
   Never hardcode a bot token in source — it's set at runtime via the
   existing generic /api/config endpoint (admin-path protected), e.g.:
     POST /{adminPath}/api/config   body: {"tg":{"token":"<BOT_TOKEN>"}}
   Then call /{adminPath}/api/tg-setup once to register the webhook.
   ------------------------------------------------------------------ */
function 获取Telegram配置() {
  return (键值配置 && 键值配置.tg) || {};
}
async function Telegram发送消息(token, chatId, text) {
  try {
...
function 随机密钥字符串(长度 = 32) {
  const 字符集 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
  let 结果 = '';
  const 随机字节 = crypto.getRandomValues(new Uint8Array(长度));
  for (let i = 0; i < 长度; i++) 结果 += 字符集[随机字节[i] % 字符集.length];
  return 结果;
}
/* Registers the webhook with Telegram. Admin-only, called once after saving a token. */
async function 处理Telegram设置(请求, 网址, 本地环境) {
  const tg配置 = 获取Telegram配置();
  if (!tg配置.token) {
    return new Response(JSON.stringify({ success: false, message: 'ابتدا توکن بات را در تنظیمات ذخیره کنید (فیلد tg.token).' }),
      { status: 400, headers: { 'Content-Type': 'application/json' } });
  }
  if (!tg配置.secret) {
    tg配置.secret = 随机密钥字符串(32);
    键值配置.tg = tg配置;
    await 保存键值配置();
  }
  const 管理路径 = (自定义路径 && 自定义路径.trim()) ? 自定义路径.trim().replace(/^\//, '') : 认证令牌;
  const webhookUrl = `${网址.origin}/${管理路径}/api/tg-webhook`;
  try {
    const resp = await fetch(`https://api.telegram.org/bot${tg配置.token}/setWebhook`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ url: webhookUrl, secret_token: tg配置.secret })
    });
    const data = await resp.json();
    return new Response(JSON.stringify({ success: !!data.ok, telegram: data, webhookUrl }),
      { headers: { 'Content-Type': 'application/json' } });
  } catch (错误) {
    return new Response(JSON.stringify({ success: false, message: String(错误 && 错误.message || 错误) }),
      { status: 502, headers: { 'Content-Type': 'application/json' } });
  }
}
/* Handles incoming updates from Telegram. Rejects anything without the
   correct secret_token header — this is the actual auth for this route. */
async function 处理Telegram网络钩子(请求) {
  const tg配置 = 获取Telegram配置();
  if (!tg配置.token || !tg配置.secret) {
    return new Response('not configured', { status: 404 });
  }
  const 收到密钥 = 请求.headers.get('X-Telegram-Bot-Api-Secret-Token');
  if (收到密钥 !== tg配置.secret) {
    return new Response('forbidden', { status: 403 });
  }
  let 更新体 = null;
  try { 更新体 = await 请求.json(); } catch (e) { return new Response('ok'); }
  const 消息 = 更新体 && 更新体.message;
  const 文本 = (消息 && 消息.text || '').trim();
  const 聊天id = 消息 && 消息.chat && 消息.chat.id;
  if (!消息 || !聊天id) return new Response('ok');

  const token = tg配置.token;
  async function 发送用户状态(条目){
    const وضعیت = 多用户是否可用(条目) ? '✅ فعال' : '⛔ غیرفعال/منقضی';
    const مصرف = ((Number(条目.usedBytes) || 0) / (1024 * 1024 * 1024)).toFixed(2);
    const سقف = 条目.quotaGB ? (条目.quotaGB + ' GB') : 'نامحدود';
    const دامنه = new URL(请求.url).origin;
    const متن = `<b>${条目.label || 'کاربر'}</b>\nوضعیت: ${وضعیت}\nانقضا: ${条目.expire || 'هرگز'}\nمصرف: ${مصرف} GB از ${سقف}\n\nلینک اشتراک شما (این را در اپلیکیشن VPN خود paste کنید):\n<code>${دامنه}/${条目.uuid}/sub</code>\n\nاپلیکیشن پیشنهادی: v2rayNG (اندروید) یا Streisand / Shadowrocket (آیفون)`;
    await Telegram发送消息(token, 聊天id, متن);
  }
  async function تلاش关联(uuid值خام){
    const uuid值 = (uuid值خام || '').trim().toLowerCase();
    const 列表 = await 获取多用户列表Async();
    const 条目 = 列表.find(项目 => 项目.uuid === uuid值);
    if (!条目) {
      await Telegram发送消息(token, 聊天id, '❌ این کد پیدا نشد. لینک را دوباره از ادمین بگیرید.');
      return false;
    }
    条目.telegramChatId = String(聊天id);
    if (使用D1模式()) { await D1写入用户(条目); } else { await 保存键值配置(); }
    await Telegram发送消息(token, 聊天id, '✅ حساب شما متصل شد.');
    await 发送用户状态(条目);
    return true;
  }
  try {
    if (文本.startsWith('/start')) {
      const پیلود = 文本.replace('/start', '').trim();
      if (پیلود) {
        await تلاش关联(پیلود);
      } else {
        await Telegram发送消息(token, 聊天id,
          'خوش آمدید! 👋\nروی لینکی که ادمین برایتان فرستاده بزنید تا حساب شما خودکار متصل شود.\n\nیا اگر کد را دارید، این را بفرستید:\n<code>/link کد-شما</code>');
      }
    } else if (文本.startsWith('/link')) {
      await تلاش关联(文本.replace('/link', ''));
    } else if (文本.startsWith('/status')) {
      const 列表 = await 获取多用户列表Async();
      const مطابقت列表 = 列表.filter(项目 => 项目.telegramChatId === String(聊天id));
      if (!مطابقت列表.length) {
        await Telegram发送消息(token, 聊天id, 'هنوز حسابی متصل نشده. روی لینک ادمین بزنید یا /link کد-شما را بفرستید.');
      } else {
        for (const 条目 of مطابقت列表) await 发送用户状态(条目);
      }
    } else {
      await Telegram发送消息(token, 聊天id, 'دستورات: /link UUID — /status');
    }
  } catch (忽略) {}
  ...
  async fetch(请求735, 本地值734, 本地值733) {
    try {
      ...
      if (网址698.pathname.includes('/api/users')) {
        const 路径部分列表_us = 网址698.pathname.split('/').filter(参数值 => 参数值);
        const 接口索引_us = 路径部分列表_us.indexOf('api');
        let 是否有效_us = false;
        if (接口索引_us > 0) {
          const 路径值_us = 路径部分列表_us.slice(0, 接口索引_us).join('/');
          if (自定义路径 && 自定义路径.trim()) {
            const 清理自定义路径_us = 自定义路径.trim().startsWith('/') ? 自定义路径.trim().substring(1) : 自定义路径.trim();
            是否有效_us = 路径值_us === 清理自定义路径_us;
          } else {
            是否有效_us = 是否有效格式(路径值_us) && 路径值_us === 认证令牌;
          }
        }
        if (!是否有效_us) {
          return new Response(JSON.stringify({ error: '路径验证失败' }), { status: 403, headers: { 'Content-Type': 'application/json' } });
        }
        return await 处理用户API(请求735);
      }
      if (网址698.pathname.includes('/api/tg-webhook')) {
        return await 处理Telegram网络钩子(请求735);
      }
      if (网址698.pathname.includes('/api/tg-setup')) {
        const 路径部分列表_tg = 网址698.pathname.split('/').filter(参数值 => 参数值);
        const 接口索引_tg = 路径部分列表_tg.indexOf('api');
        let 是否有效_tg = false;
        if (接口索引_tg > 0) {
          const 路径值_tg = 路径部分列表_tg.slice(0, 接口索引_tg).join('/');
          if (自定义路径 && 自定义路径.trim()) {
            const 清理自定义路径_tg = 自定义路径.trim().startsWith('/') ? 自定义路径.trim().substring(1) : 自定义路径.trim();
            是否有效_tg = 路径值_tg === 清理自定义路径_tg;
          } else {
            是否有效_tg = 是否有效格式(路径值_tg) && 路径值_tg === 认证令牌;
          }
        }
        if (!是否有效_tg) {
          return new Response(JSON.stringify({ error: '路径验证失败' }), { status: 403, headers: { 'Content-Type': 'application/json' } });
        }
        return await 处理Telegram设置(请求735, 网址698, 本地值734);
      }
    ...
    async write(块397) {
      ...
      if (!协议类型) {
        if (启用明文 && 数据396.byteLength >= 24) {
          let 轻量协议结果 = null;
          let 匹配令牌VLESS = null;
          for (const 候选令牌 of await 获取候选认证令牌列表()) {
            const 尝试结果 = 解析网页套接字值头部(数据396, 候选令牌);
            if (!尝试结果.hasError) { 轻量协议结果 = 尝试结果; 匹配令牌VLESS = 候选令牌; break; }
            if (!轻量协议结果) 轻量协议结果 = 尝试结果; // keep first error for fallthrough
          }
          if (轻量协议结果 && !轻量协议结果.hasError) {
            远程连接值409.matchedSubUser = 匹配令牌VLESS;
            协议类型 = 解码64('dmxlc3M=');
            const {
              addressType: 地址类型395,
              port: 端口394,
        ...
        if (启用木马 && 数据396.byteLength >= 56) {
          ...
          let 匹配令牌Trojan = null;
          for (const 候选令牌 of await 获取候选认证令牌列表()) {
            const 尝试结果 = await 解析木马头部(数据396, 候选令牌);
            if (!尝试结果.hasError) { 值结果 = 尝试结果; 匹配令牌Trojan = 候选令牌; break; }
            if (!值结果) 值结果 = 尝试结果;
          }
          if (值结果 && !值结果.hasError) {
            远程连接值409.matchedSubUser = 匹配令牌Trojan;
            协议类型 = atob('dHJvamFu');
            const {
              addressType: 地址类型388,
              port: 端口387,
  ...
  const 值页面 = `<!DOCTYPE html>
    ...
    <body>
        ...
        <div class="container">
            ...
            <div class="card">
                <h2 class="card-title">${是否值236 ? 'مدیریت کاربران' : '用户管理'}</h2>
                <div id="muStatus" style="margin-bottom: 16px; padding: 10px; background: rgba(8,10,14,0.8); border: 1px solid #c8f135; color: #c8f135; font-size: 0.85rem;">
                    ${是否值236 ? 'در حال بارگذاری...' : '正在加载...'}
                </div>
                <div id="muContent" style="display:none;">
                    <div id="muModeRow" style="margin-bottom:14px;color:#9aa6b8;font-size:0.85rem;"></div>
                    <div style="overflow-x:auto;">
                    <table style="width:100%;border-collapse:collapse;font-size:13px;margin-bottom:14px;">
                        <thead><tr style="color:#9aa6b8;text-align:${是否值236 ? 'right' : 'left'};">
                          <th style="padding:6px;">${是否值236 ? 'برچسب' : '标签'}</th>
                          <th style="padding:6px;">UUID</th>
                          <th style="padding:6px;">${是否值236 ? 'وضعیت' : '状态'}</th>
                          <th style="padding:6px;">${是否值236 ? 'انقضا' : '到期'}</th>
                          <th style="padding:6px;">${是否值236 ? 'حجم' : '流量'}</th>
                          <th style="padding:6px;">${是否值236 ? 'مصرف' : '已用'}</th>
                          <th></th>
                        </tr></thead>
                        <tbody id="muRows"></tbody>
                    </table>
                    </div>
                    <button id="muAddBtn" style="padding:10px 16px;background:#c8f135;color:#0a0f05;border:0;border-radius:6px;font-weight:bold;cursor:pointer;">+ ${是否值236 ? 'افزودن کاربر' : '添加用户'}</button>
                    <div id="muForm" style="display:none;margin-top:16px;padding:16px;border:1px solid #c8f135;background:rgba(8,10,14,0.6);">
                        <div style="margin-bottom:10px;"><label style="display:block;margin-bottom:6px;color:#c8f135;">${是否值236 ? 'برچسب' : '标签'}</label><input id="muLabel" style="width:100%;padding:8px;background:rgba(0,0,0,0.8);border:1px solid #c8f135;color:#c8f135;"></div>
                        <div style="margin-bottom:10px;"><label style="display:block;margin-bottom:6px;color:#c8f135;">UUID (${是否值236 ? 'خالی = خودکار' : '留空=自动'})</label><input id="muUuid" style="width:100%;padding:8px;background:rgba(0,0,0,0.8);border:1px solid #c8f135;color:#c8f135;"></div>
                        <div style="margin-bottom:10px;"><label style="display:block;margin-bottom:6px;color:#c8f135;">${是否值236 ? 'انقضا' : '到期日'}</label><input id="muExpire" type="date" style="width:100%;padding:8px;background:rgba(0,0,0,0.8);border:1px solid #c8f135;color:#c8f135;"></div>
                        <div style="margin-bottom:10px;"><label style="display:block;margin-bottom:6px;color:#c8f135;">${是否值236 ? 'حجم GB (۰=نامحدود)' : '流量GB(0=无限)'}</label><input id="muQuota" type="number" value="0" style="width:100%;padding:8px;background:rgba(0,0,0,0.8);border:1px solid #c8f135;color:#c8f135;"></div>
                        <label style="color:#c8f135;"><input type="checkbox" id="muActive" checked style="width:auto;"> ${是否值236 ? 'فعال' : '启用'}</label>
                        <div style="margin-top:12px;">
                          <button id="muSaveBtn" style="padding:8px 14px;background:#c8f135;color:#0a0f05;border:0;border-radius:6px;font-weight:bold;cursor:pointer;">${是否值236 ? 'ذخیره' : '保存'}</button>
                          <button id="muCancelBtn" style="padding:8px 14px;background:transparent;border:1px solid #c8f135;color:#c8f135;border-radius:6px;cursor:pointer;">${是否值236 ? 'انصراف' : '取消'}</button>
                        </div>
                        <div id="muFormMsg" style="margin-top:8px;font-size:12px;"></div>
                    </div>
                </div>
            </div>
            <script>
            (function(){
              ...
              var muEditing = -1;
              var muDbMode = 'kv';
              var muD1Available = false;
              function muApiUrl(){ return window.location.pathname + '/api/users'; }
              function muGenUuid(){
              ...
              function muLoad(){
                fetch(muApiUrl()).then(function(r){ return r.json(); }).then(function(d){
                  muUsers = Array.isArray(d.users) ? d.users : [];
                  muDbMode = d.dbMode || 'kv';
                  muD1Available = !!d.d1Available;
                  document.getElementById('muStatus').style.display = 'none';
                  document.getElementById('muContent').style.display = 'block';
                  muRenderMode();
                  muRenderRows();
                }).catch(function(e){
                  document.getElementById('muStatus').textContent = '${是否值236 ? 'خطا در بارگذاری کاربران: ' : '加载用户失败: '}' + e.message;
                });
              }
              function muRenderMode(){
                var el = document.getElementById('muModeRow');
                var modeLabel = muDbMode === 'd1' ? 'D1 (SQL)' : 'KV';
                var switchLabel = muDbMode === 'd1' ? '${是否值236 ? 'تغییر به KV' : '切换到KV'}' : '${是否值236 ? 'تغییر به D1' : '切换到D1'}';
                el.innerHTML = '${是否值236 ? 'ذخیره‌سازی: ' : '存储: '}<b style="color:#c8f135;">' + modeLabel + '</b> <button id="muModeBtn" style="margin-inline-start:8px;padding:4px 10px;background:transparent;border:1px solid #c8f135;color:#c8f135;border-radius:6px;cursor:pointer;font-size:12px;">' + switchLabel + '</button>';
                document.getElementById('muModeBtn').onclick = muSwitchMode;
              }
              function muSwitchMode(){
                var target = muDbMode === 'd1' ? 'kv' : 'd1';
                if (target === 'd1' && !muD1Available){
                  alert('${是否值236 ? 'برای D1 اول باید یک D1 database بسازید و در Bindings این Worker یک binding با نام DB اضافه کنید.' : '请先创建D1数据库并在此Worker的Bindings中添加名为DB的绑定。'}');
                  return;
                }
                fetch(muApiUrl(), { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ action:'setMode', mode: target }) })
                  .then(function(r){ return r.json(); })
                  .then(function(d){ if(!d.success) throw new Error(d.message||'failed'); muLoad(); })
                  .catch(function(e){ alert('${是否值236 ? 'خطا: ' : '错误: '}' + e.message); });
              }
              function muRenderRows(){
                var tbody = document.getElementById('muRows');
                tbody.innerHTML = '';
                if (!muUsers.length){
                  tbody.innerHTML = '<tr><td colspan="7" style="color:#9aa6b8;padding:8px;">${是否值236 ? 'هنوز کاربری اضافه نشده.' : '尚未添加用户。'}</td></tr>';
                  return;
                }
                muUsers.forEach(function(u, i){
                  var expired = u.expire && Date.parse(u.expire) < Date.now();
                  var ok = u.active !== false && !expired;
                  var usedGB = ((Number(u.usedBytes)||0) / (1024*1024*1024)).toFixed(2);
                  var tr = document.createElement('tr');
                  tr.style.borderBottom = '1px solid rgba(200,241,53,0.15)';
                  tr.innerHTML =
                    '<td style="padding:6px;">' + (u.label||'-') + '</td>' +
                    '<td style="padding:6px;font-family:monospace;font-size:11px;color:#9aa6b8;">' + u.uuid + '</td>' +
                    '<td style="padding:6px;"><span style="padding:2px 8px;border-radius:99px;font-size:11px;background:' + (ok?'rgba(61,220,132,.15)':'rgba(255,93,108,.15)') + ';color:' + (ok?'#3ddc84':'#ff5d73') + ';">' + (ok ? '${是否值236?'فعال':'启用'}' : (expired?'${是否值236?'منقضی':'过期'}':'${是否值236?'غیرفعال':'禁用'}')) + '</span></td>' +
                    '<td style="padding:6px;">' + (u.expire || '${是否值236?'هرگز':'永不'}') + '</td>' +
                    '<td style="padding:6px;">' + (u.quotaGB ? u.quotaGB : '${是否值236?'نامحدود':'无限'}') + '</td>' +
                    '<td style="padding:6px;">' + usedGB + ' GB</td>' +
                    '<td style="padding:6px;"></td>';
                  var actionsTd = tr.lastElementChild;
                  var mk = function(txt, color){ var b=document.createElement('button'); b.textContent=txt; b.style.cssText='margin-inline-start:4px;padding:4px 8px;font-size:11px;border-radius:6px;cursor:pointer;background:transparent;border:1px solid '+color+';color:'+color+';'; return b; };
                  var editBtn = mk('${是否值236?'ویرایش':'编辑'}', '#c8f135'); editBtn.onclick = function(){ muOpenForm(i); };
                  var toggleBtn = mk(u.active===false ? '${是否值236?'فعال‌سازی':'启用'}' : '${是否值236?'غیرفعال‌سازی':'禁用'}', '#c8f135'); toggleBtn.onclick = function(){ muToggle(i); };
                  var delBtn = mk('${是否值236?'حذف':'删除'}', '#ff5d73'); delBtn.onclick = function(){ muDelete(i); };
                  var subBtn = mk('${是否值236?'کپی لینک':'复制链接'}', '#5b8cff'); subBtn.onclick = function(){ muCopySub(u.uuid); };
                  actionsTd.append(editBtn, toggleBtn, delBtn, subBtn);
                  tbody.appendChild(tr);
                });
              }
              function muCopySub(uuid){
                var url = window.location.origin + '/' + uuid + '/sub';
                if (navigator.clipboard) navigator.clipboard.writeText(url).catch(function(){});
                alert('${是否值236?'کپی شد: ':'已复制: '}' + url);
              }
              function muOpenForm(index){
                muEditing = (typeof index === 'number') ? index : -1;
                document.getElementById('muForm').style.display = 'block';
                var u = muEditing>=0 ? muUsers[muEditing] : { label:'', uuid:'', expire:'', quotaGB:0, active:true };
                document.getElementById('muLabel').value = u.label||'';
                document.getElementById('muUuid').value = u.uuid||'';
                document.getElementById('muExpire').value = u.expire||'';
                document.getElementById('muQuota').value = u.quotaGB||0;
                document.getElementById('muActive').checked = u.active!==false;
                document.getElementById('muFormMsg').textContent = '';
              }
              function muToggle(i){
                var u = Object.assign({}, muUsers[i]);
                u.active = u.active===false ? true : false;
                muUpsert(u);
              }
              function muDelete(i){
                if (!confirm('${是否值236?'حذف این کاربر؟':'删除此用户？'}')) return;
                fetch(muApiUrl(), { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ action:'delete', uuid: muUsers[i].uuid }) })
                  .then(function(r){ return r.json(); })
                  .then(function(d){ if(!d.success) throw new Error(d.message||'failed'); muLoad(); })
                  .catch(function(e){ alert('${是否值236?'خطا: ':'错误: '}' + e.message); });
              }
              function muUpsert(entry){
                fetch(muApiUrl(), { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ action:'upsert', user: entry }) })
                  .then(function(r){ return r.json(); })
                  .then(function(d){
                    if(!d.success) throw new Error(d.message||'failed');
                    document.getElementById('muForm').style.display = 'none';
                    muLoad();
                  })
                  .catch(function(e){ document.getElementById('muFormMsg').textContent = '${是否值236?'خطا: ':'错误: '}' + e.message; });
              }
              document.getElementById('muAddBtn').addEventListener('click', function(){ muOpenForm(); });
              document.getElementById('muCancelBtn').addEventListener('click', function(){ document.getElementById('muForm').style.display = 'none'; });
              document.getElementById('muSaveBtn').addEventListener('click', function(){
                var entry = {
                  uuid: document.getElementById('muUuid').value.trim() || muGenUuid(),
                  label: document.getElementById('muLabel').value.trim(),
                  expire: document.getElementById('muExpire').value || null,
                  quotaGB: Number(document.getElementById('muQuota').value) || 0,
                  active: document.getElementById('muActive').checked,
                  usedBytes: muEditing>=0 ? (muUsers[muEditing].usedBytes||0) : 0
                };
                muUpsert(entry);
              });
              muLoad();
